...
- `buildPosterior(sm, data, uq_results, cm_sample; ...) -> CMPosteriorResult` —
  ...
  - `data::AbstractCMData` — real observations as a single-param-set `CMData` (`n_cm_param_sets == 1`)
  - `uq_results::Vector{<:ProfileLikelihoodResult}` — CM-side profiles, one per cm_param_set (from SmoreBase)
...
- Keyword arguments:
  - `cm_names::Vector{String}` — CM parameter names for labeling the posterior; defaults to
    `cm_sample.names` (SmoreBase's `GridCMSample`/`ScatteredCMSample` now carry names natively —
    see SmoreBase PRD.md "Feature: CM Sample" — so a separate `ParameterPrior` is no longer
    needed here just for labels). On the raw-matrix form, defaults to `nothing` and threads
    through to `CMSample(cm_params; names = cm_names)`.
...
confidence information in the same SM-parameter space as the CM param_sets' profiles. A cm_param_set is
...
- Posterior: accepted cm_param_sets (uniform sample set) by default, or normalized scores as a
...
**Acceptance criteria:** The cm_param_set that generated the data is accepted and scores
highest under all three bridge methods; cm_param_sets with disjoint SM regions are rejected;
graded scores decrease away from the best match. `cm_names` defaults to `cm_sample.names`
(auto-generated `"cm_1", ...` when built from a raw matrix without names attached) and can be
overridden explicitly. Covered in `test/runtests.jl`.
...
- **CM param_set refinement diagnostic.** Linear bound interpolation is reliable only when the bound
  surface moves a small fraction of its own width between adjacent cm_param_sets — a
  ...
  different causes make that ratio large — steep manifold, tight CIs, or coarse CM param_set spacing —
  and the interpolator can't tell them apart; in each case interior queries can return zero
  even near a cm_param_set that is in the posterior. The user's only knob is param_set density,
  ...
  users can add cm_param_sets where flagged.
